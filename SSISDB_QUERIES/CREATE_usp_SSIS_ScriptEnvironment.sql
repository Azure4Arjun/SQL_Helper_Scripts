...
DROP PROCEDURE IF EXISTS dbo.usp_SSIS_ScriptEnvironment
GO
...
*/
	   SELECT [env_var] = @tab + @tab + ', @' + ev.name + ' '  + ev.base_data_type  
			...
			-- + CASE ev.type WHEN 'String' THEN LEN(ISNULL(CONVERT(VARCHAR(MAX), ev.value), '<REPLACE_ME>')) ELSE '' END
			-- + CASE ev.type WHEN 'String' THEN CONVERT(VARCHAR(MAX), LEN(ISNULL(CONVERT(VARCHAR(MAX), ev.value), '<REPLACE_ME>'))) ELSE '' END
			-- + CASE ev.type WHEN 'String' THEN '('+CONVERT(VARCHAR(MAX), LEN(ISNULL(CONVERT(VARCHAR(MAX), ev.value), '<REPLACE_ME>')))+')' ELSE '' END
			+ CASE ev.type 
			WHEN 'String' THEN 
			
				CASE LEN(ISNULL(CONVERT(VARCHAR(MAX), ev.value), '<REPLACE_ME>'))
				WHEN 0 THEN 1 -- in case a variable is an empty String define it as VARCHAR(1) (because VARCHAR(0) would cause an error in the output script)
				ELSE LEN(ISNULL(CONVERT(VARCHAR(MAX), ev.value), '<REPLACE_ME>'))
				END
			END
